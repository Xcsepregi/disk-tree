...
{
	std::regex rgx { "^(help|list|size|quit|rm|mkdir|link|touch|tree)(\\s-(recursive|follow)(\\s-(recursive|follow)){0,1}){0,1}(\\s+(.+)){0,1}$" };
	std::smatch match;
	...
	if (match[1] == "help")
	{
		...
		if (match[7].matched)
		{
			if (match[7].str()[0] == '/' || match[7].str()[0] == '-') return {};
			param = match[7].str();
		}
	}
	...
	return std::make_pair(command, Options{ bFollow, bRecursive, param });
}
